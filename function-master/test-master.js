...
//5
  QUnit.test("Functions don't have access to eachothers scope", function(assert){
    ...
    function foo(){
      ...
      assert.equal(this.from_yay, undefined); //answer
    }
    yay();
    foo();
  });
...
//8
  QUnit.test("Inner scope can access outer scope", function(assert){
    ...
    function yay(){
      var im_inside = "omega";
      return im_outside + im_inside; //alphaoemga with no space!!!!
    }
...
//10
  QUnit.test("We can do goofy stuff with outer scope", function(assert){
    ...
    yay();
    assert.equal(name, "greggreg"); //greggreg
    yay();
    assert.equal(name, "greggreggreg"); //greggreggreg
  });
